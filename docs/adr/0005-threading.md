...
* Optional live AdvantageScope JSON/WebSocket runs on `trace-advantagescope` at `Thread.MIN_PRIORITY`.
* The loop only enqueues (writer) or updates a latest-value mailbox (live).
